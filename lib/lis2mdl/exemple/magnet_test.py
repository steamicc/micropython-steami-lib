...
def test_sets(dev):
    ...
    exp = 0b00
    print("set_mode(continuous): MD=", _bits(r,1,0), "expected", exp, "=>", "OK" if _bits(r,1,0)==exp else "FAIL"); ok &= (_bits(r,1,0)==exp)

    dev.set_mode("single")
    r = dev.read_reg(LIS2MDL_CFG_REG_A)
    exp = 0b01
    print("set_mode(single):      MD=", _bits(r,1,0), "expected", exp, "=>", "OK" if _bits(r,1,0)==exp else "FAIL"); ok &= (_bits(r,1,0)==exp)

    dev.set_mode("idle")
    r = dev.read_reg(LIS2MDL_CFG_REG_A)
    exp = 0b11
    print("set_mode(idle):        MD=", _bits(r,1,0), "expected", exp, "=>", "OK" if _bits(r,1,0)==exp else "FAIL"); ok &= (_bits(r,1,0)==exp)

    # --- ODR ---
    dev.set_odr(50)
    r = dev.read_reg(LIS2MDL_CFG_REG_A)
    exp = 0b10
    print("set_odr(50):         ODR=", _bits(r,3,2), "expected", exp, "=>", "OK" if _bits(r,3,2)==exp else "FAIL"); ok &= (_bits(r,3,2)==exp)

    dev.set_odr(100)
    r = dev.read_reg(LIS2MDL_CFG_REG_A)
    exp = 0b11
    print("set_odr(100):        ODR=", _bits(r,3,2), "expected", exp, "=>", "OK" if _bits(r,3,2)==exp else "FAIL"); ok &= (_bits(r,3,2)==exp)

    # --- Low power ---
    dev.set_low_power(True)
    r = dev.read_reg(LIS2MDL_CFG_REG_A)
    print("set_low_power(True):  LP=", (r>>4)&1, "expected 1 =>", "OK" if ((r>>4)&1)==1 else "FAIL"); ok &= (((r>>4)&1)==1)
    dev.set_low_power(False)
    r = dev.read_reg(LIS2MDL_CFG_REG_A)
    print("set_low_power(False): LP=", (r>>4)&1, "expected 0 =>", "OK" if ((r>>4)&1)==0 else "FAIL"); ok &= (((r>>4)&1)==0)

    # --- LPF ---
    dev.set_low_pass(True)
    r = dev.read_reg(LIS2MDL_CFG_REG_B)
    print("set_low_pass(True):  LPF=", r & 1, "expected 1 =>", "OK" if (r & 1)==1 else "FAIL"); ok &= ((r & 1)==1)
    dev.set_low_pass(False)
    r = dev.read_reg(LIS2MDL_CFG_REG_B)
    print("set_low_pass(False): LPF=", r & 1, "expected 0 =>", "OK" if (r & 1)==0 else "FAIL"); ok &= ((r & 1)==0)

    # --- Offset cancellation ---
    dev.set_offset_cancellation(True, oneshot=False)
    r = dev.read_reg(LIS2MDL_CFG_REG_B)
    print("set_offset_cancellation(True,False): OFF_CANC(bit1)=", (r>>1)&1, "ONE_SHOT(bit4)=", (r>>4)&1, "expected 1,0 =>",
          "OK" if ((r>>1)&1)==1 and ((r>>4)&1)==0 else "FAIL"); ok &= (((r>>1)&1)==1 and ((r>>4)&1)==0)

    dev.set_offset_cancellation(True, oneshot=True)
    r = dev.read_reg(LIS2MDL_CFG_REG_B)
    print("set_offset_cancellation(True,True):  OFF_CANC(bit1)=", (r>>1)&1, "ONE_SHOT(bit4)=", (r>>4)&1, "expected 1,1 =>",
          "OK" if ((r>>1)&1)==1 and ((r>>4)&1)==1 else "FAIL"); ok &= (((r>>1)&1)==1 and ((r>>4)&1)==1)

    # --- BDU / Endianness / SPI4 ---
    dev.set_bdu(True)
    r = dev.read_reg(LIS2MDL_CFG_REG_C)
    print("set_bdu(True):     BDU(bit4)=", (r>>4)&1, "expected 1 =>", "OK" if ((r>>4)&1)==1 else "FAIL"); ok &= (((r>>4)&1)==1)

    dev.set_endianness(True)
    r = dev.read_reg(LIS2MDL_CFG_REG_C)
    print("set_endianness(True): BLE(bit3)=", (r>>3)&1, "expected 1 =>", "OK" if ((r>>3)&1)==1 else "FAIL"); ok &= (((r>>3)&1)==1)

    dev.use_spi_4wire(True)
    r = dev.read_reg(LIS2MDL_CFG_REG_C)
    print("use_spi_4wire(True): 4WSPI(bit2)=", (r>>2)&1, "expected 1 =>", "OK" if ((r>>2)&1)==1 else "FAIL"); ok &= (((r>>2)&1)==1)
    ...
    ang1 = dev.heading_flat_only()   # (remember to add offset+declination in your method)
    print("heading_flat_only with offset+declination: angle≈raw+17° (check visually) =>", f"{ang1:.2f}°")
    ...
    print("set_calibrate_step(...): applied offsets/scales =>", (xoff,yoff,zoff,xs,ys,zs))
    ...
    oxL = dev.read_reg(LIS2MDL_OFFSET_X_REG_L); oxH = dev.read_reg(LIS2MDL_OFFSET_X_REG_L+1)
    print("set_hw_offsets(...): OFFSET_X* =", (oxH<<8)|oxL, "expected written value")
...
def test_reads(dev):
    ...
    print(f"read_magnet_raw: (X,Y,Z)=({xr},{yr},{zr}) LSB  =>", "OK" if all(isinstance(v, int) for v in (xr,yr,zr)) else "FAIL")
    ...
    ok_conv = (_approx_equal(xu, xr*0.15, 0.5) and
               _approx_equal(yu, yr*0.15, 0.5) and
               _approx_equal(zu, zr*0.15, 0.5))
    print("Conversion µT vs RAW*0.15 =>", "OK" if ok_conv else "FAIL")
    ok &= ok_conv

    # MAGNITUDE
    B = dev.magnitude_uT()
    print(f"magnitude_uT: |B|={B:.1f} µT (Earth ~25–65 µT).  =>", "OK" if 5.0 <= B <= 200.0 else "FAIL")
    ...
    ok_temp = (isinstance(t1, float) and isinstance(t2, float) and (-100.0 < t1 < 150.0) and (-100.0 < t2 < 150.0))
    print("Temp check =>", "OK" if ok_temp else "FAIL")
    ok &= ok_temp

    # INT SOURCE (without IT config, should be 0)
    ints = dev.read_int_source()
    print(f"INT_SOURCE=0x{ints:02X} (often 0 if no interrupt configured) =>", "OK" if isinstance(ints, int) else "FAIL")
    ok &= isinstance(ints, int)

    # REGISTER DUMP (sanity)
    dump = dev.read_registers(LIS2MDL_CFG_REG_A, 8)  # A..H ~ 0x60..0x67
    print(f"Dump 0x60..0x67: {dump} =>", "OK" if isinstance(dump, (bytes, bytearray)) and len(dump)==8 else "FAIL")
...
def test_calibrate_2d(dev):
    ...
    xoff,yoff,_, xs,ys,_ = dev.x_off, dev.y_off, dev.z_off, dev.x_scale, dev.y_scale, dev.z_scale
    ...
    print("anisotropy_xy =", "{:.2f}".format(q["anisotropy_xy"]), " (≈1.0 if circle is nicely round)")
...
def test_calibrate_3d(dev):
    ...
    q = dev.calibrate_quality(samples_check=200, delay_ms=10)
    print("mean_xy =", _fmt_tuple(q["mean_xy"]), "  mean_z = {:.3f}".format(q["mean_z"]))
    print("std_xy  = ({:.3f},{:.3f})  std_z = {:.3f}".format(q["std_xy"][0], q["std_xy"][1], q["std_z"]))
    print("anisotropy_xy =", "{:.2f}".format(q["anisotropy_xy"]))
    print("r_mean_xy =", "{:.3f}".format(q["r_mean_xy"]), "  r_std_xy =", "{:.3f}".format(q["r_std_xy"]))
...
def test_heading_after_calib(dev, n=200, delay_ms=20):
    ...
    for _ in range(n):
        ...
        angles.append(ang)
        sleep_ms(delay_ms)
    minA = min(angles); maxA = max(angles); span = (maxA - minA) % 360.0
...
def run_all_calibration_tests(dev):
    ...
    print("\n=== Calibration summary:", "OK ✅" if (ok2d and ok3d) else "Partial ⚠️ (see WARN/notes)")
...
def test_heading_offset_declination(dev):
    ...
    print(f"angle0={a0:.2f}°, angle1={a1:.2f}°, diff≈{diff:.2f}°  =>", "OK" if ok else "FAIL")
    return ok
...
def test_heading_span_turn(dev, duration_ms=6000, step_ms=50):
    ...
    dev.set_heading_offset(0.0); dev.set_declination(0.0)
    angles = []
    t = 0
    while t < duration_ms:
        a = dev.heading_flat_only()
...
def test_heading_filter_wrap(dev):
    ...
    dev._hf_cos = None; dev._hf_sin = None  # reset filter
...
def test_power_modes(dev):
    ...
    r = dev.read_reg(LIS2MDL_CFG_REG_A)
    md = _bits(r,1,0)
    print("wake('continuous') => MD=", md, "expected 0b00 =>", "OK" if md==0b00 else "FAIL"); ok &= (md==0b00)

    # Wake in single
    dev.wake("single")
    r = dev.read_reg(LIS2MDL_CFG_REG_A); md = _bits(r,1,0)
    print("wake('single')      => MD=", md, "expected 0b01 =>", "OK" if md==0b01 else "FAIL"); ok &= (md==0b01)

    # Power down
    dev.power_down()
    r = dev.read_reg(LIS2MDL_CFG_REG_A); md = _bits(r,1,0)
    print("power_down()        => MD=", md, "expected 0b11 =>", "OK" if md==0b11 else "FAIL"); ok &= (md==0b11)
    print("is_idle():", dev.is_idle(), "expected True =>", "OK" if dev.is_idle() else "FAIL"); ok &= dev.is_idle()
...
def test_soft_reset(dev):
    ...
    print(f"Before reset:  CFG_A=0x{ra_before:02X} CFG_B=0x{rb_before:02X} CFG_C=0x{rc_before:02X}")
    ...
    print("MD=idle (11) =>", "OK" if md_ok  else "FAIL"); ok &= md_ok
    print("ODR=00       =>", "OK" if odr_ok else "FAIL"); ok &= odr_ok
    print("LP=0         =>", "OK" if lp_ok  else "FAIL"); ok &= lp_ok
    print("LPF=0        =>", "OK" if lpf_ok else "FAIL"); ok &= lpf_ok
    print("BDU=0        =>", "OK" if bdu_ok else "FAIL"); ok &= bdu_ok

    # Verify that the SOFT_RST bit has cleared back to 0 (auto-clear)
    soft_rst_cleared = ((ra >> 5) & 1) == 0
    print("SOFT_RST auto-clear =>", "OK" if soft_rst_cleared else "FAIL"); ok &= soft_rst_cleared
...
def test_reboot(dev):
    ...
    print("REBOOT auto-clear =>", "OK" if reboot_cleared else "FAIL"); ok &= reboot_cleared

    # WHO_AM_I still correct
    who = dev.read_who_am_i()
    print(f"WHO_AM_I=0x{who:02X} expected 0x40 =>", "OK" if who==0x40 else "FAIL"); ok &= (who==0x40)
